...
export const StartPopup: React.FC<{ 
  ...
  return (
    <div className="absolute inset-0 bg-black/85 flex flex-col items-center justify-center p-2 md:p-4 text-center z-40 touch-auto overflow-hidden">
      ...
      <h1 className="text-xl md:text-5xl text-white mb-2 md:mb-3 pixel-font animate-bounce drop-shadow-[0_2px_0_#2980b9] leading-tight">CLEAR AHEAD</h1>
      
      <div className="flex justify-center gap-2 mb-2 md:mb-4 scale-90 md:scale-100">
      ...
      <div className="flex flex-col gap-2 md:gap-4 w-full max-w-lg bg-white/5 p-3 md:p-6 rounded-xl md:rounded-2xl border border-white/10 backdrop-blur-sm">
        <div className="flex items-center justify-between gap-2 border-b border-white/10 pb-2 md:pb-3">
        ...
        <div className="w-full">
          ...
          <div className="grid grid-cols-2 gap-1 md:gap-2">
            <button onClick={() => { audioService.playButtonClick(); onModeChange('EASY'); }} className={`py-1.5 md:py-2 px-1 rounded-lg border-b-2 md:border-b-4 pixel-font text-[8px] md:text-xs transition-all ${selectedMode === 'EASY' ? 'bg-[#2ecc71] border-[#1e8449] text-white' : 'bg-gray-700 border-gray-900 text-gray-400 opacity-60'}`}>EASY</button>
            <button onClick={() => { audioService.playButtonClick(); onModeChange('HARD'); }} className={`py-1.5 md:py-2 px-1 rounded-lg border-b-2 md:border-b-4 pixel-font text-[8px] md:text-xs transition-all ${selectedMode === 'HARD' ? 'bg-[#e67e22] border-[#a04000] text-white' : 'bg-gray-700 border-gray-900 text-gray-400 opacity-60'}`}>HARD</button>
          </div>
        </div>
            ...
            {(['FOREST', 'SNOW', 'ROAD', 'BEACH'] as MapType[]).map((m) => {
              ...
              return (
                <button 
                  ...
                  className={`relative py-1.5 md:py-2.5 px-0.5 rounded-lg border-b-2 md:border-b-4 pixel-font text-[6px] md:text-[10px] transition-all ${selectedMap === m ? 'bg-[#3498db] border-[#21618c] text-white shadow-[0_0_5px_rgba(52,152,219,0.4)]' : 'bg-gray-700 border-gray-900 text-gray-400 opacity-60'} ${!isOwned ? 'grayscale' : ''}`}
                >
                  {m}
                  {!isOwned && (
...
export const ShopPopup: React.FC<{ 
  ...
  return (
    <div className="absolute inset-0 bg-black/95 flex flex-col items-center p-2 md:p-4 z-50 overflow-hidden touch-auto">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-2 md:gap-4 w-full max-w-4xl flex-1 overflow-hidden">
        {(activeTab === 'CHARS' ? CHARACTER_DATA : MAP_DATA).map(item => {
          ...
          return (
            <div key={item.id} className={`flex flex-col items-center justify-between p-2 md:p-3 rounded-lg border-2 bg-white/5 transition-all ${isSelected ? (activeTab === 'CHARS' ? 'border-purple-500 bg-purple-500/10' : 'border-blue-500 bg-blue-500/10') : 'border-white/10'}`}>
